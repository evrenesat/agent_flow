...
from extensions.ralf_universal.controller import (
    ControllerError,
    build_system_prompt,
    build_user_prompt,
    run_controller,
)
from extensions.ralf_universal.harnesses.claude import ClaudeAdapter
from extensions.ralf_universal.harnesses.codex import CodexAdapter
from extensions.ralf_universal.harnesses.pi import PiAdapter
from extensions.ralf_universal.plan import PlanParseError, load_plan
from extensions.ralf_universal.run_state import ControllerConfig
from extensions.ralf_universal.runlog import create_run_paths, prune_old_runs
...
def _copy_universal_repo(tmp_path: Path) -> Path:
    ...
    scripts_dir.mkdir()
    launcher = scripts_dir / "ralf-universal"
    launcher.write_text((REPO_ROOT / "scripts" / "ralf-universal").read_text(encoding="utf-8"), encoding="utf-8")
...
def _run_launcher(repo_root: Path, *args: str, env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [str(repo_root / "scripts" / "ralf-universal"), *args],
        cwd=repo_root,
        env=env,
        capture_output=True,
        text=True,
...
class AdaptersTests(unittest.TestCase):
    def test_adapters_codex_prefixes_controller_prompt_into_user_prompt(self) -> None:
    ...
    def test_adapters_pi_uses_system_prompt_and_full_tool_list(self) -> None:
        adapter = PiAdapter()
        invocation = adapter.build_invocation(
            repo_root=Path("/repo"),
            model="sonnet",
    ...
    def test_adapters_claude_uses_bypass_permissions_flags(self) -> None:
...
""",
            ...
            def runner(argv, **kwargs):
                nonlocal calls
                calls += 1
                # Flip the checkpoint title every turn so the snapshot changes,
                # but the plan never becomes complete.
                if calls % 2:
                    title = "Alpha"
                else:
                    title = "Beta"
...
class EndToEndLauncherTests(unittest.TestCase):
    def test_launcher_completes_with_fake_codex_and_writes_turn_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            repo_root = _copy_universal_repo(tmp_path)
